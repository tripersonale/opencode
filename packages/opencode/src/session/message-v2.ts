...
// position is not chronological. Derive each binding by max position so a
// pre-compaction overflowing tail assistant doesn't get mistaken for the most
// recent turn. tasks are compaction/subtask parts attached to user messages
// newer than the latest finished assistant — i.e. unprocessed work.
//
// Ordering is (time.created, id): MessageID.ascending changed prefix scheme
// across the 2026-08 fork restore (msg_f… → msg_00…), so plain id string
// comparison silently mis-orders sessions mixing pre/post-restore messages.
export function msgAfter(a: { time: { created: number }; id: string }, b: { time: { created: number }; id: string }) {
  return a.time.created > b.time.created || (a.time.created === b.time.created && a.id > b.id)
}
...
export function latest(msgs: WithParts[]) {
  ...
  for (const msg of msgs) {
    const info = msg.info
    if (info.role === "user" && (!user || msgAfter(info, user))) user = info
    if (info.role === "assistant" && (!assistant || msgAfter(info, assistant))) assistant = info
    if (info.role === "assistant" && info.finish && (!finished || msgAfter(info, finished))) finished = info
  }
  const tasks = msgs.flatMap((m) =>
    finished && !msgAfter(m.info, finished)
      ? []
